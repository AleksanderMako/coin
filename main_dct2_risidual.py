...
for i in range(min_id, max_id + 1):
    ...
    trainer.dct2_risidual(img,args.num_iters,k,coordinates)
    ...
    with torch.no_grad():
        ...
        risidual_recon = func_rep(coordinates).reshape(img.shape[1], img.shape[2], 3).permute(2, 0, 1)
        mask = torch.zeros_like(img)
        mask[:k,:k] = 1 
        frequency_domain, mask = frequency_domain.to(device), mask.to(device)
        base_frequencies = frequency_domain*mask
        dct_recon = apply_idct2(base_frequencies)
        img_recon = dct_recon+risidual_recon
