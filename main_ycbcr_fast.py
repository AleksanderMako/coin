...
for i in range(min_id, max_id + 1):
    ...
    trainer.train_ycbcr_with_exploit(img,num_iters=args.num_iters)
    ...
    with torch.no_grad():
        coordinatesY,coordinatesUV, chroma_sub_2,featuresY,featuresUV,chroma,_ = trainer.get_coords_from_image(img)
        y_hat,cbcr_hat = func_rep(coordinatesY,coordinatesUV)
        img_recon = trainer.build_image(y_hat,cbcr_hat,img,chroma_sub_2)
    ...
    if torch.cuda.is_available():
        ...
        coordinatesY,coordinatesUV, chroma_sub_2,featuresY,featuresUV,chroma,_ = trainer.get_coords_from_image(img)
        coordinatesY = coordinatesY.half().to('cuda')
        coordinatesUV = coordinatesUV.half().to('cuda')
